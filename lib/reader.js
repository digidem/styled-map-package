...
export default class Reader {
  /** @type {Promise<import('yauzl-promise').ZipFile> | undefined} */
  #zipOpenPromise
  /** @type {Promise<Map<string, import('yauzl-promise').Entry>> | undefined} */
  #entriesPromise
  ...
  constructor(filepathOrZip) {
    this.#filepathOrZip = filepathOrZip
  }

  /**
   * @returns {Promise<import('yauzl-promise').ZipFile>}
   */
  async #openZip() {
    return typeof this.#filepathOrZip === 'string'
      ? open(this.#filepathOrZip)
      : Promise.resolve(this.#filepathOrZip)
  }
  ...
  async #getEntries() {
    ...
    if (!this.#zipOpenPromise) {
      this.#zipOpenPromise = this.#openZip()
    } else {
      try {
        await this.#zipOpenPromise
      } catch {
        // Each time getEntries is called, we should try to open the zip again
        this.#zipOpenPromise = this.#openZip()
      }
    }
    const zip = await this.#zipOpenPromise
  ...
  async getStyle(baseUrl = null) {
    const styleEntry = (await this.#getEntries()).get(STYLE_FILE)
  ...
  async getResource(path) {
    ...
    const entry = (await this.#getEntries()).get(path)
